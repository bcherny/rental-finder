...
const seq = require('promise-seq')

const chunk = lodash.chunk
const flatten = lodash.flatten
const partition = lodash.partition
...
// (void) => Object[String]
module.exports.getProgress = function getProgress () {
	return state.progress
}

// (area: String, subareas: Array[String]) => Promise[Array[House]]
module.exports.fetch = function fetch (area, subareas) {
	return fetchPosts(area, subareas)
		...
		.then(hs => hs.filter(h => h.PostingID).map(h => { return {
			lat: h.Latitude,
			lng: h.Longitude,
